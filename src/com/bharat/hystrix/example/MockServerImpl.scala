...
class MockServerImpl {
  ...
  def createExpectation(request:HttpRequest):Unit ={
    clientAndServer.when(request).callback(new HttpCallback().withCallbackClass("com.bharat.hystrix.example.MockCallBackHandler"))
  }

}

  class MockCallBackHandler extends ExpectationCallback{
    ...
    override def handle(httpRequest: HttpRequest): HttpResponse = {
      ...
      if("success".equalsIgnoreCase(x.getValues.get(0).getValue)){
        ...
        new HttpResponse().withDelay(new Delay(TimeUnit.SECONDS,2)).withBody("hello").applyDelay()
      }
